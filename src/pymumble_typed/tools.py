...
class InvalidVarIntError(Exception):
...
class VarInt:
    ...
    def decode(self, value):
        ...
        if len(varint) <= 0:
            raise InvalidVarIntError("length can't be 0")
        ...
        if first & 0b11111100 == 0b11111000:
            ...
            size += 1
            if len(varint) < 2:
                raise InvalidVarIntError("Too short negative varint")
            varint = varint[1:]
            (first, ) = struct.unpack("!B", varint[0:1])
        ...
        if first & 0b10000000 == 0b00000000:
            ...
            if len(varint) < 2:
                raise InvalidVarIntError("Too short 2 bytes varint")
            (result, ) = struct.unpack("!H", varint[:2])
            result &= 0b0011111111111111
            size += 2
        elif first & 0b11100000 == 0b11000000:
            if len(varint) < 3:
                raise InvalidVarIntError("Too short 3 bytes varint")
            (result, ) = struct.unpack("!B", varint[0:1])
            result &= 0b00011111
            ...
            if len(varint) < 4:
                raise InvalidVarIntError("Too short 4 bytes varint")
            (result, ) = struct.unpack("!L", varint[:4])
            result &= 0x0fffffff
            size += 4
        elif first & 0b11111100 == 0b11110000:
            if len(varint) < 5:
                raise InvalidVarIntError("Too short 5 bytes varint")
            (result, ) = struct.unpack("!L", varint[1:5])
            size += 5
        elif first & 0b11111100 == 0b11110100:
            if len(varint) < 9:
                raise InvalidVarIntError("Too short 9 bytes varint")
